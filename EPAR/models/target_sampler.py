...
class TargetSampler:
    ...
    def select_embed_random(
        ...
        for _ in range(self._num_targets_per_sample):
            ...
            selected_indices = torch.randperm(T, device=self._device, generator=self._rng)[:num_to_select]
    ...
    def select_embed_contiguous(
        ...
        for _ in range(self._num_targets_per_sample):
            ...
            start_index = torch.randint(0, max_start_index + 1, (1,), device=self._device, generator=self._rng).item()
    ...
    def sample(
        ...
        """
        with torch.no_grad():
            ratio = torch.rand(1, requires_grad=False, generator=self._rng, device=self._device) * (

...
class MaskCollator(object):
    ...
    def _sample_block_mask(self, 
        ...
        while not valid_mask:
            ...
            cnt = torch.isin(mask, background_indices)
            count = cnt.sum().item()
            # -- check whether mask contains sufficient foreground patches
            # -- If mask too small, try again
            # print(f"Length of mask: {len(mask)} and Min keep pred size: {min_keep_pred}")
            if len(mask) >= min_keep_pred:
                if (len(mask) - count) / len(mask) >= threshold:
                    valid_mask = True
            if not valid_mask:
                timeout -= 1
                if timeout == 0:
                    tries += 1
                    min_keep_pred = len(mask)
        mask = mask.squeeze()
        # --
    ...
    def __call__(self, 
        ...
        for b in range(B):
            masks_p = []
            background_idx = np.array(background_indices[b])

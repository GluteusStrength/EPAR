...
class Attention(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.unsqueeze(1).unsqueeze(1)  # shape: [bsz, 1, 1, npatches]
            attn = attn.masked_fill(attention_mask==0, float(-1e9))

        attn = attn.softmax(dim=-1) # shape: [bsz, n_heads, npatches, npatches]
...
class VisionTransformerPredictor(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.predictor_embed = nn.Linear(embed_dim, predictor_embed_dim, bias=False)
        self.mask_token = nn.Parameter(torch.zeros(1, 1, predictor_embed_dim)) # Learnable Parameter
    ...
    def forward(self, x, masks_x, masks, device, attention_mask=None):
        ...
        x += apply_masks(x_pos_embed, masks_x, device=device) # Add positional embedding on context blocks indices 
        ...
        x = torch.cat([x, pred_tokens], dim=1) # bsz*num_predictors, n_context + n_targets, dim
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.patch_embed = nn.Linear(feature_dim, embed_dim, bias=False)
    ...
    def forward(self, x, masks=None, attention_mask=None):
        ...
        # mask on unmeaningful tokens.

        # -- mask x
        if masks is not None:
            x = apply_masks(x, masks, device=x.device)
            # # select attention mask
            # attention_mask = attention_mask.gather(dim=1, index=masks[0])
...
def vit_small(patch_size=16, depth=12, modality=None, drop_rate=0.0, feature_dim=768, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, modality=modality, feature_dim=feature_dim, embed_dim=384, depth=depth, num_heads=6, mlp_ratio=4, drop_rate=drop_rate,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def vit_base(patch_size=16, depth=12, modality=None, drop_rate=0.0, feature_dim=768, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, modality=modality, feature_dim=feature_dim, embed_dim=768, depth=depth, num_heads=12, mlp_ratio=4, drop_rate=drop_rate,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

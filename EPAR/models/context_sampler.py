...
class ContextSampler:
    def __init__(
        ...
        self._rng.manual_seed(42)  # Set a fixed seed for reproducibility
    ...
    def get_context_random_idx(self, valid_indices: torch.Tensor ,num_context_selct: int) -> torch.Tensor:
        ...
        """
        selected_indices = torch.randperm(len(valid_indices), device=self._device, generator=self._rng)[:num_context_selct]
    ...
    def get_context_contiguous_idx(self, valid_indices: torch.Tensor ,num_context_selct: int) -> torch.Tensor:
        ...
        start_index = torch.randint(0, max_valid_start + 1, (1,), device=self._device, generator=self._rng)
    ...
    def sample(
        ...
        ratio = (torch.rand(1, requires_grad=False, device=self._device, generator=self._rng) *
        ...
        return context_blocks_tokens, context_blocks_centers, context_indices
...
"""
This script preprocesses the MVTec 3D-AD dataset by removing planes from the point clouds, padding the images to make them square, and cleaning up connected components in the point clouds. 
The processed files are saved back to their original locations.
Before running this script, ensure that the MVTec 3D-AD dataset is downloaded and available at the specified path.
The script takes the root path of the dataset as an argument and processes all .tiff files in the dataset.
The preprocessing steps include:
1. Reading the point cloud and RGB images from the dataset.
2. Removing planes from the point clouds using RANSAC.
3. Padding the images to make them square by adding zeros to the shorter side.
4. Cleaning up connected components in the point clouds using DBSCAN clustering.
5. Saving the preprocessed point clouds and RGB images back to their original locations.
The script uses the Open3D library for point cloud processing and the PIL library for image handling.
The script is designed to be run from the command line, and it requires the path to the MVTec 3D-AD dataset as an argument.
The script also includes a function to round up the dimensions of the images to the nearest hundred, ensuring that the final images are square and have dimensions that are multiples of 100.
The script is intended for use in a machine learning or computer vision context, where the preprocessed point clouds and images can be used for training and evaluation of models.
"""
...
def get_plane_eq(unorganized_pc,ransac_n_pts=50):
    ...
    if len(o3d_pc.points) == 0:
        print("No points in the point cloud. Returning default plane model.")
        assert False
    else:
...
if __name__ == '__main__':
    ...
    processed_files = 0
    for path in tqdm(Path(root_path).rglob('*.tiff')):
        preprocess_pc(path)
        processed_files += 1

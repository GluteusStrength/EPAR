    ...
    def forward(self, points: torch.Tensor):
        ...
        for dim in self.dims:
            if self.deg is not None:
                ...
                nonzero_degs = possible_degs[possible_degs != 0]
                euler_angles[dim] = nonzero_degs[   
                    torch.randint(high=len(nonzero_degs), size=(1,))
                ]
            else:
                rand_val = torch.rand(1).clamp(min=1e-6, max=1 - 1e-6)
                euler_angles[dim] = rand_val * 2 * torch.pi

        R = euler_angles_to_matrix(euler_angles.unsqueeze(0), "XYZ").squeeze(0)

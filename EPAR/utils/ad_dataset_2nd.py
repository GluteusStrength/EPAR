...
class TrainDataset(AnomalyDetectionDataset):
    ...
    def __getitem__(self, idx):
        ...
        # Perlin noise based mask
        gray_tensor = self.grayscale_transform(img_tensor)
        object_mask = (gray_tensor > DARK_THRESHOLD).float() # [1, H, W]
        perlin_mask = self.generate_perlin_mask(
            size=(self.img_size, self.img_size), 
            threshold=0.7, 
            scale=80 # best: 0.7, 80 combination
        )                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
        final_anomaly_mask = object_mask * perlin_mask
        anomaly_mask = final_anomaly_mask
        anomaly_label = 1

        # DTD Source Image
        if not self.dtd_paths:
            source_idx_rgb = random.randint(0, len(self.img_paths) - 1)
            source_rgb_path, _ = self.img_paths[source_idx_rgb]
            raw_img_source = Image.open(source_rgb_path).convert('RGB')
        else:
            dtd_idx = random.randint(0, len(self.dtd_paths) - 1)
            dtd_path = self.dtd_paths[dtd_idx]
            raw_img_source = Image.open(dtd_path).convert('RGB')

        augmented_source_pil = self.source_transform(raw_img_source)
        
        # Pseudo Anomaly (RGB)
        augmented_source_rgb_tensor = self.rgb_transform(augmented_source_pil)
        source_patch_rgb = augmented_source_rgb_tensor * final_anomaly_mask
        target_with_hole_rgb = img_tensor * (1 - final_anomaly_mask)
        rgb_anomaly_tensor = target_with_hole_rgb + source_patch_rgb
            
        # Texture anomaly (Depth)
        augmented_source_gray_pil = self.dtd_gray_transform(augmented_source_pil)
        augmented_source_gray_tensor = self.rgb_transform(augmented_source_gray_pil) 
        source_patch_depth = augmented_source_gray_tensor * final_anomaly_mask
        target_with_hole_depth = depth_tensor * (1 - final_anomaly_mask)
        depth_anomaly_tensor_texture = target_with_hole_depth + source_patch_depth
        
        # Geometric Anomaly
        source_idx_pc = random.randint(0, len(self.img_paths) - 1)
        _, source_tiff_path = self.img_paths[source_idx_pc]
        source_organized_pc = read_tiff_organized_pc(source_tiff_path)
        source_resized_organized_pc = resize_organized_pc(source_organized_pc, target_height=self.img_size, target_width=self.img_size).clone().detach().float()

        # Geometric Anomaly (3D Point Cloud)
        source_pc_patch = source_resized_organized_pc * final_anomaly_mask
        translation_vector = (torch.rand(3, 1, 1) - 0.5) * 0.1 
        augmented_source_pc_patch = source_pc_patch + (translation_vector * final_anomaly_mask) 
        target_pc_with_hole = resized_organized_pc * (1 - final_anomaly_mask)
        xyz_anomaly_tensor = target_pc_with_hole + augmented_source_pc_patch
        
        # Geometric Anomaly (Depth)
        anomalous_z_channel = xyz_anomaly_tensor[2, :, :].unsqueeze(0) # [1, H, W]
        depth_anomaly_tensor_geometric = anomalous_z_channel.repeat(3, 1, 1) # [3, H, W]

            
        return {
            'data': (img_tensor, resized_organized_pc, depth_tensor, label),
            "anomaly_data": (rgb_anomaly_tensor, xyz_anomaly_tensor, depth_anomaly_tensor_texture, depth_anomaly_tensor_geometric, anomaly_label, anomaly_mask, object_mask),
            'label': label 
        }
